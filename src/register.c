...
// #lizard forgives
void register_to_remote_with_data(const char *bus_id, const char *pod_uid, const char *container) {
    pid_t register_pid;
    ...
    if (!register_pid) {
        ...
        if (is_custom_config_path()) {
            ret = execl((RPC_CLIENT_PATH RPC_CLIENT_NAME), RPC_CLIENT_NAME, "--addr", RPC_ADDR, "--bus-id", bus_id, "--pod-uid", pod_uid, "--cont-id", container, (char *)NULL);
        }
        else {
            ret = execl((RPC_CLIENT_PATH RPC_CLIENT_NAME), RPC_CLIENT_NAME, "--addr", RPC_ADDR, "--bus-id", bus_id, "--pod-uid", pod_uid, "--cont-name", container, (char *)NULL);
        }
        if (unlikely(ret == -1)) {
