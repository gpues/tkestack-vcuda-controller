...
void sort(unsigned int *nds) { // 升序
    for (int i = 0; i <= 14; ++i) {
        for (int j = i + 1; j <= 15; ++j) {
            if (nds[i] > nds[j]) {
                unsigned int v4 = nds[i];
                nds[i] = nds[j];
                nds[j] = v4;
            }
        }
...
int64_t parser(char *NVIDIA_DEVICE_MAP_VALUE) {
    ...
    if (haystack && !strstr(haystack, "all")) { // 不是 all
    ...
    while (nptr) {
        ...
        if (found) {
            ...
            if (strstr(uuidPtr, "GPU")) { // strstr(str1,str2) 函数用于判断字符串str2是否是str1的子串
...
int64_t initial_virtual_devices() {
    ...
    for (int i = 0; i < virtual_devices[0]; ++i) {
        ...
        if (isfirst(record_nvml_map[i])) {
            ...
            v53 = strtol((const char *)&vdevices[i].handle, &endptr, 16) + record_nvml_map[i] + 1; //     char str[] = "12345"; -> long int 12345
