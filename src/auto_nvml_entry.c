...
nvmlReturn_t nvmlDeviceGetPciInfo_v3(nvmlDevice_t device, nvmlPciInfo_t *pci) {
    return NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetPciInfo_v3, device, pci);
}
...
nvmlReturn_t nvmlDeviceGetProcessUtilization(nvmlDevice_t device, nvmlProcessUtilizationSample_t *utilization, unsigned int *processSamplesCount, unsigned long long lastSeenTimeStamp) {
    return NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetProcessUtilization, device, utilization, processSamplesCount, lastSeenTimeStamp);
}
...
nvmlReturn_t nvmlGpmQueryDeviceSupport(nvmlDevice_t device, nvmlGpmSupport_t *gpmSupport) {
    return NVML_ENTRY_CALL(nvml_library_entry, nvmlGpmQueryDeviceSupport, device, gpmSupport);
}

nvmlReturn_t nvmlDeviceCcuGetStreamState(nvmlDevice_t device, unsigned int *state) {
    return NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceCcuGetStreamState, device, state);
}

nvmlReturn_t nvmlDeviceCcuSetStreamState(nvmlDevice_t device, unsigned int state) {
    return NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceCcuSetStreamState, device, state);
}
...
nvmlReturn_t nvmlDeviceGetHandleByIndex(unsigned int index, nvmlDevice_t *device) {
    return NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetHandleByIndex, index, device);
}
...
nvmlReturn_t nvmlDeviceGetPciInfo_v2(nvmlDevice_t device, nvmlPciInfo_t *pci) {
    return NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetPciInfo_v2, device, pci);
}
...
nvmlReturn_t nvmlDeviceGetComputeRunningProcesses(nvmlDevice_t device, unsigned int *infoCount, nvmlProcessInfo_v1_t *infos) {
    return NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetComputeRunningProcesses, device, infoCount, infos);
}
