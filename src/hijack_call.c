...
const int cuda_hook_nums = sizeof(cuda_hooks_entry) / sizeof(cuda_hooks_entry[0]);
...
static void atomic_action(const char *filename, atomic_fn_ptr fn_ptr, void *arg) {
    int fd;
...
static void *utilization_watcher(void *arg UNUSED) {
    ...
    while (1) {
        ...
        LOGGER(4, "util: %d, up_limit: %d,  share: %d, cur: %d", top_result.user_current, up_limit, share, g_cur_cuda_cores);
    }
}
...
static void get_used_gpu_utilization(int fd, void *arg) {
    ...
    ret = NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetHandleByIndex, 0, &dev);
    if (unlikely(ret)) {
        LOGGER(4, "nvmlDeviceGetHandleByIndex: %s", nvml_error(ret));
        return;
    }

    ret = NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetComputeRunningProcesses, dev, &running_processes, pids_on_device);
    if (unlikely(ret)) {
    ...
    ret = NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetProcessUtilization, dev, processes_sample, &processes_num, microsec);
    if (unlikely(ret)) {
...
static void get_used_gpu_memory(int fd, void *arg) {
    ...
    ret = NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetComputeRunningProcesses, dev, &size_on_device, pids_on_device);
    if (unlikely(ret)) {
        LOGGER(
            4,
    ...
    for (i = 0; i < size_on_device; i++) {
        LOGGER(4, "summary: %d used %lld", pids_on_device[i].pid, pids_on_device[i].usedGpuMemory);
    }

    for (i = 0; i < size_on_device; i++) {
        if (bsearch(&pids_on_device[i].pid, g_pids_table, (size_t)g_pids_table_size, sizeof(int), int_match)) {
            LOGGER(4, "%d use memory: %lld", pids_on_device[i].pid, pids_on_device[i].usedGpuMemory);
...
static void register_to_remote() {
    ...
    ret = NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetHandleByIndex, 0, &nvml_dev);
    if (unlikely(ret)) {
        LOGGER(FATAL, "can't find device 0, error %s", nvml_error((nvmlReturn_t)ret));
    }

    ret = NVML_ENTRY_CALL(nvml_library_entry, nvmlDeviceGetPciInfo, nvml_dev, &pci_info);
    if (unlikely(ret)) {
        LOGGER(FATAL, "can't find device 0, error %s", nvml_error((nvmlReturn_t)ret));
    }

    strncpy(g_vcuda_config.bus_id, pci_info.busId, NVML_DEVICE_PCI_BUS_ID_BUFFER_SIZE);

    register_to_remote_with_data(g_vcuda_config.bus_id, g_vcuda_config.pod_uid, g_vcuda_config.container_name);
}
...
static void initialization() {
    ...
    ret = CUDA_ENTRY_CALL(cuda_library_entry, cuDeviceGetAttribute, &g_sm_num, CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT, 0);
    if (unlikely(ret)) {
        LOGGER(FATAL, "can't get processor number, error %s", cuda_error((CUresult)ret, &cuda_err_string));
    }

    ret = CUDA_ENTRY_CALL(cuda_library_entry, cuDeviceGetAttribute, &g_max_thread_per_sm, CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR, 0);
    if (unlikely(ret)) {
        LOGGER(FATAL, "can't get max thread per processor, error %s", cuda_error((CUresult)ret, &cuda_err_string));
    }
...
CUresult cuGetProcAddress(const char *symbol, void **pfn, int cudaVersion, cuuint64_t flags) {
    ...
    ret = CUDA_ENTRY_CALL(cuda_library_entry, cuGetProcAddress, symbol, pfn, cudaVersion, flags);
    if (ret == CUDA_SUCCESS) {
...
CUresult cuArrayCreate_v2(CUarray *pHandle, const CUDA_ARRAY_DESCRIPTOR *pAllocateArray) {
    ...
    ret = CUDA_ENTRY_CALL(cuda_library_entry, cuArrayCreate_v2, pHandle, pAllocateArray);
DONE:
    return ret;
}
...
CUresult cuMipmappedArrayCreate(CUmipmappedArray *pHandle, const CUDA_ARRAY3D_DESCRIPTOR *pMipmappedArrayDesc, unsigned int numMipmapLevels) {
    ...
    ret = CUDA_ENTRY_CALL(cuda_library_entry, cuMipmappedArrayCreate, pHandle, pMipmappedArrayDesc, numMipmapLevels);
DONE:
    return ret;
}
...
CUresult cuLaunchKernel_ptsz(CUfunction f, unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ, unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ, unsigned int sharedMemBytes, CUstream hStream, void **kernelParams, void **extra) {
    ...
    return CUDA_ENTRY_CALL(cuda_library_entry, cuLaunchKernel_ptsz, f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream, kernelParams, extra);
}
...
CUresult cuLaunchCooperativeKernel_ptsz(CUfunction f, unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ, unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ, unsigned int sharedMemBytes, CUstream hStream, void **kernelParams) {
    rate_limiter(gridDimX * gridDimY * gridDimZ, blockDimX * blockDimY * blockDimZ);
    return CUDA_ENTRY_CALL(cuda_library_entry, cuLaunchCooperativeKernel_ptsz, f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream, kernelParams);
}
