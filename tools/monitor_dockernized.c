...
int main(void) {
    ...
    for (i = 0; i < device_num; i++) {
        ...
        for (j = 0; j < size_on_device; j++) {
            LOGGER(4, "summary: %d used %lld", pids_on_device[j].pid, pids_on_device[j].usedGpuMemory);
        }
        ...
        for (j = 0; j < processes_num; j++) {
            LOGGER(4, "summary: %d util %d", processes_sample[j].pid, processes_sample[j].smUtil);
        }
